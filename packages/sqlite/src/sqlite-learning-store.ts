...
export class SqliteLearningStore implements LearningStore {
  ...
  public async findItem(
    ...
    const row = this.#database
      ...
      `)
      .get(id, registeredAlgorithmId) as Row | undefined;
  ...
  public async listItems(
    ...
    validatePage(query);
    const parameters: SQLInputValue[] = [registeredAlgorithmId];
    let sql = `
      SELECT item.id, item.data_json
      FROM engine_items item
      JOIN engine_states state ON state.learning_item_id = item.id
  ...
  public async commitReview(input: {
    ...
    return this.transaction(() => {
      if (
        input.state.state.learningItemId !== input.review.learningItemId ||
        input.state.state.algorithmId !== input.review.algorithmId
      ) {
      ...
      const update = this.#database
        ...
        .run(
          ...
          input.state.state.revision,
        );
      if (toSafeInteger(update.changes) !== 1) {
        throw new ConcurrentReviewError(input.review.learningItemId);
      }
  ...
  public async listReviews(
    ...
    validatePage(query);
    const parameters: SQLInputValue[] = [itemId, registeredAlgorithmId];
    let sql = `
      SELECT
